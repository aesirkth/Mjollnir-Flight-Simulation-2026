...
if child_nr == numel(children); if isstruct(structure.(child)) && isscalar(structure.(child)); branch_string = "╘"; else; branch_string = "└"; end; new_stem = stem + " " + string(repmat(' ', 1, max_characters-1));
else;                           if isstruct(structure.(child)) && isscalar(structure.(child)); branch_string = "╞"; else; branch_string = "├"; end; new_stem = stem + "│" + string(repmat(' ', 1, max_characters-1)); 
end

if isstruct(structure.(child)) && isscalar(structure.(child))
